...
class OpenAIController(BaseLLMController):
    def __init__(self, model: str = "gpt-4", api_key: Optional[str] = None, api_base: Optional[str] = None):
        try:
            ...
            self.client = OpenAI(
                ...
                timeout=float(os.getenv("PM_LLM_TIMEOUT", "300")),
            )
        except ImportError:
    ...
    def get_completion(self, prompt: str, response_format: dict, temperature: float = 0.7) -> str:
        ...
        def _call(active_kwargs):
            response = self.client.chat.completions.create(**active_kwargs)
            message = response.choices[0].message
            content = getattr(message, "content", None)
            if content:
                return content
            # Thinking / reasoning gateways sometimes leave content empty and
            # put the JSON in reasoning_content (or similar).
        ...
        try:
            content = _call(kwargs)
        except Exception as exc:
            # Environment adapter: DeepSeek rejects json_schema.
            err = str(exc).lower()
            if "json_schema" in err or "response_format" in err or "invalid" in err:
                kwargs["response_format"] = {"type": "json_object"}
                content = _call(kwargs)
            else:
                raise

        # Retry once with plain json_object if schema path yielded empty body.
        if not content and kwargs.get("response_format", {}).get("type") == "json_schema":
    ...
    def analyze_content(content: str, llm_controller: LLMController) -> Dict:            
        ...
        try:
            ...
            try:
                ...
                response = re.sub(r'^```json\s*|\s*```$', '', str(response), flags=re.MULTILINE).strip()
                analysis = json.loads(response)
            except Exception as _e:
                print(f"JSON parsing error in analyze_content: {_e}")
...
def _coerce_link_indices(raw_links, *, n_memories: int) -> list:
    """Normalize LLM link suggestions to integer list indices.

    Upstream A-Mem mixes prompt wording ("neighbor_memory_ids") with a JSON
    schema that asks for integers. Models frequently emit strings / UUIDs;
    storing those breaks ``all_memories[neighbor]`` at recall time.
    """
    out = []
    seen = set()
    for item in raw_links or []:
        idx = _as_list_index(item)
        if idx is None or idx < 0 or idx >= n_memories or idx in seen:
            continue
        seen.add(idx)
        out.append(idx)
    return out
...
def _as_list_index(value):
    """Accept plain ints and numpy integer scalars; reject bool/UUID noise."""
    if isinstance(value, bool):
        return None
    if isinstance(value, int):
        return value
    # numpy.int64 etc. are not subclasses of int
...
def _resolve_linked_note(neighbor, all_memories, memories_by_id):
    """Resolve a stored link that may be an int index or a memory-id string."""
    idx = _as_list_index(neighbor)
    if idx is not None:
        if 0 <= idx < len(all_memories):
            return all_memories[idx]
        return None
    if isinstance(neighbor, str) and neighbor in memories_by_id:
        return memories_by_id[neighbor]
    return None
...
class AgenticMemorySystem:
    ...
    def process_memory(self, note: MemoryNote) -> bool:
        ...
        try:
            if not response or not str(response).strip():
                return False, note
            # Clean the response in case there's extra text
            response_cleaned = str(response).strip()
            # Try to find JSON content if wrapped in other text
            if not response_cleaned.startswith('{'):
                start_idx = response_cleaned.find('{')
                if start_idx != -1:
        ...
        if should_evolve:
            ...
            for action in actions:
                if action == "strengthen":
                    ...
                    # Paper prompt shows "neighbor_memory_ids" (strings) while the
                    # schema asks for integer list indices. Models often emit
                    # either; only keep resolvable integer indices.
                    note.links.extend(
                        _coerce_link_indices(suggest_connections, n_memories=len(self.memories))
                    )
                    note.tags = new_tags
    ...
    def find_related_memories_raw(self, query: str, k: int = 5) -> List[MemoryNote]:
        """Paper retrieval: top-k hits, then each hit's link neighbors (≤k).

        Format matches the AgenticMemory benchmark layer: flat string with
        timestamp / content / context / keywords / tags (no product seen_ids
        dedup). Link values are resolved via index-or-id helpers so LLM-emitted
        UUID/string links do not crash recall.
        """
        if not self.memories:
            return []
        ...
        memory_str = ""
        for i in indices:
            idx = _as_list_index(i)
            if idx is None or idx < 0 or idx >= len(all_memories):
                continue
            ...
            j = 0
            for neighbor in note.links or []:
                linked = _resolve_linked_note(neighbor, all_memories, self.memories)
                if linked is None:
                    continue
                memory_str += (
                    "talk start time:"
