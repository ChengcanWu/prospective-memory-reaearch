...
```text
intent_id, summary, action_text
status:        pending | done | canceled
trigger_kind:  time | event | channel
trigger_event  事件/channel 线索（when 后面那段）
target_time    HH:MM
cross_day, target_day, created_day
channels       观察名单（appointment_portal / email / …）
source         plan | vignette | daily | encode
```

另有一份 **channel 账本** `channel_records`：上次 `query_state` / `check_time` 看到的原文、查询时刻、有效期、status（valid / stale / superseded / expired）。
...
解析 day / 当前 vignette / 本步时钟
        ↓
intention.recall()     ← 更新库 + 拼注入文本
        ↓
决策模型只看到 compact 当前步 + 注入
        ↓
模型返回 choose / check_time / query_state
        ↓
若时钟未知且有时间类 pending → 代码把动作改写成 check_time
        ↓
仅 choose 才 encode（标 done + 抽新意图）
```

记忆失败不中断该步，记 log，当空记忆继续。
...
- 当天 `=== Day ===` 头（intention：**只留这一行**，丢掉 `Today's loose plan` 的原始 when 条款；Mem0/A-Mem 仍保留完整日计划）
- **当前步**从 vignette 起的消息（含这一步里刚查到的 `State [clock]`）

日计划仍由 `_maybe_seed_day_plan` 从完整 transcript 入库；choose 不再同时看到「早上那张未改写的清单」和「库里已 override 的 event/due」。周一查过的 clock dump **不会**出现在周二的 compact prompt 里。然后追加 `INTENTION_MEMORY_INJECT_HEADER`（怎么读这块记忆）+ `recall()` 返回的看板。
...
### 4.1 记下本步的 channel 回复

`_ingest_state_replies(..., current_step_only=True)`  
只看当前 vignette **之后**的 `Time:` / `State [channel]: ...`。周一查过的 clock 不会在周二被再记成「刚刚查过」。

新内容走 `_record_channel`：

- 非 clock：默认 **Channel Expiry Judge**（LLM）给 `ephemeral / short / day / sticky` 和 TTL
- LLM 关了或失败：启发式（email/calendar 约 15 分钟，portal 90 分钟，snapshot 180 分钟，`(no updates)` 算 short 30 分钟）
- 同一天同一条 content 不重复问 LLM

然后 `_refresh_channel_expiry`：换日 → expired；TTL 过了 → stale。
...
| summary                                 | kind  | 线索                     |
| --------------------------------------- | ----- | ---------------------- |
| Take antibiotic at breakfast / dinner   | event | `breakfast` / `dinner` |
| Take asthma medication at 11:00 / 21:00 | time  | `11:00` / `21:00`      |


库里已有 summary 词袋重叠 ≥ **0.72** 的意图就改它，不新建。这是词袋，不是 LLM。
...
2. **不**再把 `due.day` 已过的 pending 标 `canceled`。换日不清库存。
3. 从 plan 文本 `_extract_from_text` 抽意图

注入仍隐藏 **未来** `due.day`（星期下标大于今天）。空 due、当天 due、已经过了的 leftover 都进 today pending。

### 4.4 抽意图：正则（LLM extract 开关目前是空的）

`use_llm_extract` 在 runner 里默认 on，但 `_extract_from_text` **没有读这个开关**。当前抽取是：

- `On Wednesday, Email the room change…` → `_CROSS_DAY_RE`，标 `cross_day`，`target_day=Wednesday`
- 行首 `- bullet` → 一条意图
- **非 bullet、挤在一段里的多个任务不会被拆开**

`_parse_intention_line` 怎么定 `trigger_kind`：


| 条件                       | kind                               |
| ------------------------ | ---------------------------------- |
| 有 `at HH:MM`             | `time`，记下 `target_time`            |
| 有 `when …` 且词表命中 channel | `channel`                          |
| 只有 `when …`              | `event`，`trigger_event` = when 后整段 |
| 只有 channel 词             | `channel`                          |
| 都没有                      | 仍标 `event`（线索可为空）                  |


channel 词表 `CHANNEL_HINTS` 是**子串**：`"email"`、`"calendar"`、`"portal"`、`"below $"` 等。句子里出现这些词就会挂 watchlist。

写入一律 `_insert`：不按 action overlap 合并。日计划里两条相同 bullet、或 extract 抽到和 daily 药同名的条目，都会各留一行。daily 四条药换日时用**精确** `action` + `due.time` / `event_cue` 重开同一槽，不是抽取去重。

### 4.5 Scene Judge：改期 / 取消 / 换线索 + 本步到期

`apply_observation_updates(observation, messages=…)`，**recall 和 encode 都会跑**。同一条 `(day, time, observation, State)` 有缓存；encode 无新 State 时不重问。

**一次 LLM**：全部 pending，今天的排前面。模型回：
...
没有修订就回 `"updates": []`。点名对不上 `n` 就失败。Scene Judge 的 update 半边必须开着，没有正则后手。`--no-intention-llm-match` 忽略 due 标签，仍把全部 today pending 注入。
...
TIME-DUE 只是注入里的日历式提醒。未命中的时钟行仍进 **TODAY pending**，choose 自己决定是否做。session **不会**丢掉未到期时钟任务的菜单点选。

同时：`today_pending` 里只要还有时间类意图、且时钟为空 → `force_check_time=True`。哮喘药几乎天天 pending，所以**几乎每步开头都会强制查钟**。这是走 PM-Bench 合法的 `check_time` 通道，不是把时间印在 vignette 上。

### 4.8 EVENT：Scene Judge 的 event_due（不是子串 EVENT-CUED）

`use_llm_match` 默认开。同一 Scene Judge 看本步 vignette，标 `event_due` / `event_wait`。未命中的 event 行从 choose 注入删除。`--no-intention-llm-match` 则仍把全部 EVENT pending 给 choose。
...
无本步 `State [channel]` → `channel_wait`，**不删**，CHECK 软名单，choose 自己决定是否 `query_state`（不强制）。已有 State 且 Judge 判未到期 → 从注入删除。命中 → `[CHANNEL-DUE]`。

### 4.10 对齐当前步菜单：choose 模型

**不再**用 `_align_to_menu` 词袋（曾 overlap ≥ 0.34 才注入，对不上的 pending 整条丢掉）。`TODAY pending` = 不是 TIME-DUE、也不是 EVENT 的剩余 pending（含未对上钟的时钟行）。
...
4. `CHECK channels: email, calendar, ...`（软名单）
5. Match 开：`EVENT due` / `CHANNEL due`（Judge 命中 + 同线索 fan-out）+ `CHECK`（无 State 的 wait）+ 其余 `TODAY pending`。未命中的 event / 已查未到期的 watch **不出现**。共享 `event_cue` 的 due 行收在同一个 `[cue]` 组下。
6. Match 关：`EVENT pending`（软提醒，不是 EVENT-CUED 盖章；同线索仍分组）+ 其余 today pending

单条形如：

```text
- [PENDING] Pick up the replacement filter when the follow-up text arrives | trigger=event | event=the follow-up text | status=pending
```
...
choose 的 `task_ids` 不再按 TIME-DUE 过滤；未到期时钟任务是否执行由模型决定。
...
1. **再跑一遍** Scene Judge 的 updates 半边（通常命中缓存）
2. 把 `task_ids` 经当前 menu 译成人类标签（`task_14` → `Take asthma medication at 11:00`）
3. **Done Judge**：把这些标签和编号 pending 交给模型，只按序号 `n` 标 `done`。无菜单动作则跳过。无 LLM 则失败，没有 overlap 后手
4. **Extract Judge** 从 vignette 抽新意图（日计划种子仍走 `_extract_from_text`）
...
以 `intention_store.py` / `session.py` / `common.py` 为准。Judge 对不上序号或没开 LLM 会 `_fail`，没有词袋/正则后手。
...
| 时钟 | 要不要先查钟 | 代码 | 本步无钟 **且** 存在带 `due.time` 的 today pending → `force_check_time`。session 把非 `check_time` / `query_state clock` 的动作**整份改写成** `check_time`（`--no-force-check-time` 可关） |
...
| 时钟 | 未到期的时钟任务能不能做 | **choose 模型** | TIME-DUE 只是软标签。session 不丢掉未到期时钟任务的菜单点选 |
| 查询 | CHECK 里出现哪些名字 | 代码 | `_watch_needed`：today pending 的 `watch`，去掉 `clock`、去掉本步已出现的 `State [x]` / `Time:` |
| 查询 | 要不要 `query_state` 业务 channel | **choose 模型** | **先于** channel due。due 依赖隐藏状态且本步还没有 `State [channel]` → 应先 `query_state`（注入头第 3 条）。CHECK 只是软名单，**不强制**（强制查业务 channel 等于公开隐藏通道） |
| 查询 | 本步已经查过哪些 channel | 代码 | `_queried_channels_this_step`：当前 vignette **之后**的 user 行。`Time:` → `clock`；`State [name]:` → 该 name |
| 到期 | 事件任务是否现在到期 | **Scene Judge LLM** + 代码 fan-out | 看本步 vignette 是否出现 `event=`；**不是** EVENT-CUED 子串。Judge 命中后按库存 `event_cue` 把同一线索的其它 pending 一并 due。未命中从注入删除。`--no-intention-llm-match` 则仍全部 EVENT pending 给 choose |
| 到期 | channel 相关 pending 是否 due | **Scene Judge LLM** | 只认本步 `State [channel]`。无 State → wait，**不删**（choose 可 query）。有 State 且未到期 → 从注入删除。不强制 `query_state` |
...
| 到期 | pending 是否对应当前菜单 | **choose 模型** | 按 meaning 对齐本步 action menu。库**注入全部** today pending，不再 `_align_to_menu` |
| 写入  | vignette 抽新意图                | **Extract Judge LLM** | `_extract_observation`；必须开着，无正则后手。按 `(day, scene)` 缓存，同一步二次 recall 不重抽                                                                                             |
...
| 上下文 | 注入过长                         | 代码                    | `truncate(..., max_inject_chars)`                                                                                                                                  |
| 故障  | 记忆 / Judge 失败                | 代码                    | session `recall_error` **记 log 后 raise**（当前不是空记忆继续）。Judge `_fail` 同样中断该步                                                                                           |
...
事件到期的子串匹配、CHANNEL-DUE、菜单词袋闸、入库 upsert、标 done 的 0.4 overlap 已经删了。同类手法还在：
...
| 强制 check_time | 有时间 pending 且本步无钟               |
...
| 文件                               | 内容                                     |
| -------------------------------- | -------------------------------------- |
| `{run}.intentions.json`          | 当前整库（intentions + channel_records）     |
...
| Extract Judge                          | 开          | 接通。`--no-intention-llm-extract` 同样 `_fail`，vignette **无**正则后手（日计划种子仍走 `_extract_from_text`） |
